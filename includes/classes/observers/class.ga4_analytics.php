...
class ga4_analytics extends base
{
    ...
    protected function getID()
    {
        ...
        if (isset($_REQUEST['products_id'])) {
            if (is_array($_REQUEST['products_id'])) {
                $id = explode(':', $_REQUEST['products_id'][0]);
            } else {
                $id = explode(':', $_REQUEST['products_id']);
            }
        } else {
            if (isset($_REQUEST['product_id'])) {
                if (is_array($_REQUEST['product_id'])) {
                    $id = explode(':', $_REQUEST['product_id'][0]);
                } else {
                    $id = explode(':', $_REQUEST['product_id']);
                }
            }
        }
        $id = (int)$id[0];
    ...
    protected function getCatString($id)
    {
        global $db, $cPath;
            ...
            while ($i < count($p)) {
                $the_categories_name = $db->Execute(
                    "SELECT categories_name
                      ...
                      WHERE categories_id = '" . $p[$i] . "'
                        AND language_id = " . (int)$_SESSION['languages_id'] . "
                      LIMIT 1"
                 );
                if ($masterCat ==  $the_categories_name->fields['categories_name']) {
                    $flag = 1;
                }
                $i++ ;
            } 
    ...
    protected function addProductItemsStr()
    {
        $itemsStr = '';
        $i = 0; 
        ...
        switch ($notifier) {
            case 'NOTIFY_HEADER_START_PRODUCT_INFO':
                ...
                if ($id) {
                    ...
                    $analytics['item'] = [
                        'productID' => $id,
                        'productName' => addslashes($products_name),
                        'category' => $this->getCatString($id),
                        'brand' => $brandTxt
                    ];
            ...
            case 'NOTIFIER_CART_REMOVE_END':
                ...
                if ($notifier === 'NOTIFIER_CART_ADD_CART_END') {
                    ...
                    if (isset($_REQUEST['cart_quantity'])) {
                        $qty = (int)$_REQUEST['cart_quantity'];  // @TODO does this need to handle decimals?
                    }
                    if (!$qty) {
                        $qty = 1;
                    }
                ...
                if ($order_summary['order_number']) {
                  // add other cases as you wish for your other payment modules       
                    ...
                    $analytics['transaction'] = [
                        'id' => (string)$order_summary['order_number'],
                    ...
                    $items_query =
                        "SELECT DISTINCT orders_products_id, products_id, products_name, products_model, final_price, products_tax, products_quantity
                    ...
                    $i = 0;
                    $analytics['addProductItemsStr']= '';
                    while (!$items_in_cart->EOF) {
                        $variant = $db->Execute(
                            "SELECT products_options_values
                               FROM " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . "
                              WHERE orders_products_id = " . (string)$items_in_cart->fields['orders_products_id']
                        );
